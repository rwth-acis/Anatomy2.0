<?php 
	/**
	 * @file confirm_registration.php
	 * 
   * Sets the confirmation flag in the database for a given email address
	 */
  
  $mail = $_POST['mail'];
  
  // Get DB connection
  $conn = require '../php/db_connect.php';
  require '../config/config.php';

  // Test whether the account is already confirmed
  $sql_select = "SELECT * FROM users WHERE email='" . $mail . "' AND confirmed=1";
  $sth = $db->prepare($sql_select);
  $sth->execute();
  $user = $sth->fetch();
  // If user is not empty, the account is already confirmed
  if($user) {
    $result = array('result'=>'error', 'mail'=>$mail, 'sql_select'=>$sql_select, 'error'=>'The account for the email address \'' . $mail . '\' is already confirmed.' );	
  }
  else { 
    // Confirm account
    $sql = "UPDATE users SET confirmed=1 WHERE email='" . $mail . "'";
    $sth = $db->prepare($sql);
    $success = $sth->execute();
    
    if ($success) {
      // Send a mail to the lecturer to inform that his or her account can now be used
      $mail_to = $mail;
      $subject = "Anatomy 2.0 Account unlocked";
      $login_link = $baseUrl . "/src/views/login.php";
      $message = "Hello " . $mail_to . ",\n\n" .
        "your Anatomy 2.0 account has been unlocked. You can now start using it by logging in at: " . $login_link . "\n\n" .
        "This mail was automatically generated by the Anatomy 2.0 system. Please do not respond to this mail.";
      mail($mail_to, $subject, $message);
    
      // Create result to transmit to client. SQL is sent only for debugging
      $result = array('result'=>'ok', 'mail'=>$mail, 'sql'=>$sql, 'sql_select'=>$sql_select);		
    }
    else {
      $result = array('result'=>'error', 'mail'=>$mail, 'sql'=>$sql, 'sql_select'=>$sql_select, 'error'=>'Confirmation was not successful. Please try again.');		
    }
  }

  // JSON encode the return value to be transmitted to client again
  echo json_encode($result);
?>
	
